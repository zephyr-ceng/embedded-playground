...
2. IIC配置注意事项
   ...
   - 重试机制
   - 主从机的收发机制不一样，所以主从机均要编写收发函数
      - 主机接收需要判定最后一个字节，并且及时发送Stop信号
      - 从机接收数据后，最后一个字节不需要处理，硬件会自动处理，并且STOPF标志位不能通过clearFlag清除
   - 部分外设（从机）能够自动发送数据和接收数据，只需要主机定时的读取和发送data
