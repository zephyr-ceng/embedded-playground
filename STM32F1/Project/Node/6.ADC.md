...
- 功能框图讲解：
  ...
  3. 转换顺序：规则通道由SQRx寄存器控制，注入通道由JSQR寄存器控制（类似于优先级配置）

  4. 触发源：可以通过配置寄存器CR2、内部定时器、外部IO进行ADC转换触发

  5. 转换时间：每次ADC的信号转换均需要时间，该时间由APB2的PLCK2(最大12MHz)以及寄存器ADC_SMPR1 和 ADC_SMPR2决定()，
