...
#include "./BSP_Drive/inc/USART_DMA.h"
...
// Function to configure GPIO for USART
void USART_GpioConfig(USART_TypeDef *USARTx, uint32_t BauRate)
{
    if (USARTx == USART1) {
...
// Function to configure USART DMA for transmission
...
// Function to configure USART DMA for reception
...
// Function to configure NVIC for USART interrupts
...
void USART_DMA_Init(USART_TypeDef *USARTx, uint32_t BaudRate)
{
    ...
    USART_GpioConfig(USARTx, BaudRate);
