...
int main(void)
{
   ...
   /* USER CODE END 1 */
    ...
    HAL_TIM_CLEAR_FLAG(&htim1, TIM_FLAG_UPDATE); // 清除计数器的更新中断标志，避免残留标志影响计数
...
static void MX_TIM1_Init(void)
{
    ...
    htim1.Init.Period = 10;                                         // 设置计数器周期为 9，表示每 10 个脉冲触发一次溢出
    ...
    sSlaveConfig.InputTrigger = TIM_TS_TI1FP1; // 使用 TI1 的滤波后脉冲信号作为触发源
    sSlaveConfig.TriggerPolarity = TIM_TRIGGERPOLARITY_RISING; // 检测上升沿触发计数
