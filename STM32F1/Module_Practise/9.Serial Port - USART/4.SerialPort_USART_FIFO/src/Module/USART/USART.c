...
void SerialPort_USART_Config(uint32_t BAUDRATE)
{
    /* 定义一个 USART 结构体 */
...
// 将数据写入FIFO
static void SerialPort_FIFO_Write(uint8_t *data, uint16_t length)
{
    uint16_t i;
    // 空指针校验
...
void DEVICE_IRQHANDLER(void)
{
    ...
    if (USART_GetITStatus(DEVICE_USARTX, USART_IT_IDLE) != RESET) {
        ...
        if (len > 0 && len < BUFFER_SIZE)
            SerialPort_FIFO_Write(dma_rx_buf, len);
...
// 读取FIFO缓冲区数据
uint16_t SerialPort_FIFO_Read(uint8_t *data, uint16_t length)
{
    uint16_t i;
    if (length == 0 || data == NULL)
        return 0;
...
void SerialPort_USART_Init(uint32_t BAUDRATE)
{
    SerialPort_GPIO_Config();
    SerialPort_USART_Config(BAUDRATE);
    SerialPort_DMA_Config();
    SerialPort_NVIC_Config();
}

uint16_t SerialPort_ReceiveData(uint8_t *data, uint16_t length)
{
    return SerialPort_FIFO_Read(data, length);
}
