...
```scss
(unsigned) char
char str='h'  char string[] = "hello world!!"
(unsigned) int
(unsigned) short
(unsigned) long
float
...
- 数组长度计算：`sizeof()`
  ...
  // 长度计算
  int prices_length = sizeof(prices)/sizeof(int);  // 5
  ```
...
### 输入输出
   ...
   - 在 C 中，我们有三种类型的 I/O 流：
     ...
     - `stderr`（标准错误）错误内容屏幕输出
       ...
       int main() {
           int a;
           fprintf(stdout, "请输入一个整数：");  // 等价于 printf()
           fscanf(stdin, "%d", &a);              // 等价于 scanf()
           fprintf(stdout, "你输入的是：%d\n", a);
