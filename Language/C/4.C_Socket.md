...
- 网络字节序：参见大小端模式，为了解决数据解析问题，所以需要对字节进行转换，linux中有`srpa/inet.h`：
  ...
  ```C
  uint32_t htonl(uint32_t hostlong); // host to networklong 大端
  uint16_t htons(uint16_t hostshort); // host to hostshort 大端
  uint32_t ntohl(uint32_t netlong); // network to hostlong
  uint16_t ntohs(uint16_t netshort); // network to hostshort
  ```
...
int main(void)
{
    ...
    listenfd = socket(AF_INET, SOCK_STREAM, 0); // 1.创建服务器端套接字文件
    // 初始化服务器端口地址
    ...
    bind(listenfd, (struct sockaddr *)&servaddr, sizeof(servaddr)); // 2.将套接字文件与服务器端口地址绑定
    listen(listenfd, 20); // 3.监听，并设置最大连接数未20
    ...
    while (1)
    {
        ...
        connfd = accept(listenfd, (struct sockaddr *)&cliaddr, &cliaddr_len); // 3.等待客户端连接
        n = recv(connfd, buf, MAXLINE, 0); // 接收连接客户端发的数据
        printf("received from %s at PORT %d\n",
               inet_ntop(AF_INET, &cliaddr.sin_addr, str, sizeof(str)),
               ntohs(cliaddr.sin_port)); // inet_ntop 将网络字节序转换为标准IP地址
        for (i = 0; i < n; i++)
            buf[i] = toupper(buf[i]); // 接收的字符串转为大写
        send(connfd, buf, n, 0);      // 4.将处理后的数据发送回客户端
        close(connfd); 				  // 5.关闭连接
    }
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    sockfd = socket(AF_INET, SOCK_STREAM, 0);// 1.创建客户端套接字文件
    bzero(&servaddr, sizeof(servaddr));// 初始化服务器端口地址
    servaddr.sin_family = AF_INET; // 设置地址族为IPv4
    inet_pton(AF_INET, "127.0.0.1", &servaddr.sin_addr); // 设置服务器IP地址,将IP地址转换为网络字节序地址
    servaddr.sin_port = htons(SERV_PORT); // 设置服务器端口号
    connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr)); // 2.请求链接
    send(sockfd, str, strlen(str), 0);// 4.发送数据
    n = recv(sockfd, buf, MAXLINE, 0);// 5.接收客户端返回的数据
