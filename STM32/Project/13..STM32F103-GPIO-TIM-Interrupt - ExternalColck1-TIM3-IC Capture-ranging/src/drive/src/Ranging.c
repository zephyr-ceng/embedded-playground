...
volatile uint32_t riseTime      = 0; // 上升沿时间
volatile uint32_t fallTime      = 0; // 下降沿时间
volatile uint16_t captureStatus = 0; // 测量状态
...
void TIM3_CH_Config()
{
    ...
    TIM_ICIinitStructure.TIM_ICFilter    = 15;                       // 滤波器设置
    ...
    // 5.配置输入捕获映射
    TIM_SelectInputTrigger(TIM3, TIM_TS_ITR1); // TIM3 的IT1为触发源
...
void Trigger_Ultrasonic(){
    GPIO_SetBits(GPIOA,GPIO_Pin_6); // 发送一个高电平
    Delay_us(10);
