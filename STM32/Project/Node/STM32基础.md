...
8. TIMxCLK(CK_INT)定时器
    ...
    2. 基本定时器时基单元工作流程 `中断频率 = 输入时钟频率 / [(PSC + 1) × (ARR + 1)]`
        - 进入预分频器（`PSC`）,**主要决定计数器计数速度**，对72MHz的RCC内部时钟进行分频,APB1默认分频系数为2，实际的分频值 = 预分频值 + 1；
        - 计数器计数，分为向上计数，向下计数，中央对齐计数；
        - 自动重装初始值(`ARR`)，配合RSC可以决定中断周期，最大为65535，**当计数器的值达到设定值后产生中断溢出**
        - PSC和ARR选择问题：a. 先根据所需中断周期确定总分频系数（`PSC×ARR`） b. 权衡分辨率（ARR越大，调节越精细）和溢出风险（16位定时器ARR最大65535）
