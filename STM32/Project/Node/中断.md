...
- 基本定时器时基单元工作流程
  ...
  - 中断时间：T = 中断周期 x 中断次数;
    ...
    {
        /* 1) 使能 AFIO 时钟（如果需要重映射） */
        RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);
        
        /* 2) 使能定时器 TIM6 所在总线时钟（TIM6 在 APB1） */
    ...
    void TIM6_Configuration(void)
    {
        ...
        /* 计数频率计算
         *  假设 PCLK1 = 36MHz（默认 HCLK=72MHz, APB1 prescaler=2, TIM clock = PCLK1*2 = 72MHz）
         *  要实现 1kHz 的定时（1ms 触发一次），需要：
         *    TimerCounterClock = 72MHz / (Prescaler + 1)
         *  取 Prescaler = 7199，则计数时钟 = 10000 Hz
         *    ARR = (TimerCounterClock / UpdateEventFreq) - 1
         *    = (10000 / 1000) - 1 = 9
         */
        TIM_TimeBaseStructure.TIM_Period = 9;                  // 自动重装值（ARR）
