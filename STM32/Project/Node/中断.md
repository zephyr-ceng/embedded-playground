...
### 外部时钟2-更新事件中断（ETR 中断计数）

```C
// 设置将初级定时器替换为TIM2的ETR引脚， 对应PA0; TIM3.TIM4均没有ETR引脚
...
除去外部时钟模式还有其他模式，**这些模式需要通过更新中断`TIM_IT_update`者监听从模式标志位`TIM_Flag_Trigger`** ，主要通过两个函数确定他们分别为
...
- `void TIM_SelectSlaveMode(TIM_TypeDef* TIMx, uint16_t TIM_SlaveMode)` 第二个参数表示模式的选择，分别为：
  ...
  -  @arg TIM_SlaveMode_Reset（复位模式）:上升沿重新触发
     ...
     @arg TIM_SlaveMode_External1（外部时钟1模式）: 上升沿计数

```C
// 按输入捕获配置函数TIM_Comfig, 然后使用更新事件监听函数,前使用一下函数：

TIM_SelectInputTrigger(TIM3,TIM_TS_TI1FP1); // 选择时钟信号源
TIM_SelectSlaveMode(TIM,TIM_SlaveMode_Gated); // 模式选择
