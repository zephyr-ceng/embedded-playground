...
uint8_t code Column_Value[] = {0xaa, 0x00, 0x00, 0x7f, 0x00, 0x00, 0x00, 0x00}; // 数字 1 码值
...
void MatrixLED_Send(uint8_t column, uint8_t column_Value)
{
     ...
     *@returnValue: 无
     **/
    SendByte_74HC595_column(column);
    SendByte_74HC595_Value(column_Value);
    Out595();
    delayMs(1);
    // 发送0x00可以去除重影
    SendByte_74HC595_column(0x00);
    SendByte_74HC595_Value(0x00);
    Out595();
}
/* 初始化 */
void sendByte_Init()
void main(void)
{
    ...
    while (1) {
        for (i = 0; i < 8; i++) {
            MatrixLED_Send(Column_Test[i], Column_Value[i]);
        }
    }
}