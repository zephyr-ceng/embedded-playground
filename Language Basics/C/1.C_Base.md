...
2. 字符类型（``%d %f %c %s`）
   ...
   - const 表示常量 ``const int num=15`，大写表示常量，小写表示变量
     ...
     | ------------- | ---------------------------------- | ---- | ---------------- |
     | %u            | 十进制无符号整数                   | %x   | 十六进制整数     |
     | ***%f, %lf*** | ***float浮点数, double点数***      | %#x  | 0x形式的十六进制 |
     | %5.2f         | 右对齐宽度为5，保留2位小数的浮点数 | %lu  | 32位无符号整数   |
     | %-10f         | 宽度为10的浮点数                   | %llu | 64位无符号整数   |
     | ***%s***      | ***字符串***                       | %g   | 科学计数         |
     | %e            | 指数形式的浮点数                   | %%   | 百分号           |
     | ***%c***      | ***单个字符***                     | %p   | 指针的值         |
...
4. 运算符
   ...
   - 符合运算符： +=  -=  *=   /=   %=
   - 三目运算符： <条件> ? <表达式> : <表达式>
   - `sizeof` 运算符返回你传入的操作数的大小。你可以传入变量，或者甚至是类型也可以。如`sizeof(int)`返回字节数
   - (数据类型) 常量；强制类型转换，转换时会导致失真
   - 位运算符：&，|，^，~，<<，>>
